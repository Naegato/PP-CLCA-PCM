...
describe('Advisor Reject Loan', () => {
  ...
  const getData = (currentUser: User) => {
    ...
    const useCase = new AdvisorRejectLoan(loanRequestRepository, security);
  ...
  test('Should reject loan request successfully', async () => {
    ...
    const rejectedLoan = result as LoanRequest;
    expect(rejectedLoan.approved).toBe(false);
    expect(rejectedLoan.advisor?.identifier).toBe(advisor.identifier);
  });
  ...
  test('Should update loan request in repository with rejection', async () => {
    const advisor = createTestAdvisor();
    const client = createTestClient();
    const { useCase, loanRequestRepository } = getData(advisor);
    ...
    expect(savedLoanRequest?.approved).toBe(false);
    expect(savedLoanRequest?.advisor).toBeDefined();
  });
});
