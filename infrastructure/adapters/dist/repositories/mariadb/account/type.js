...
export class MariaDbAccountTypeRepository {
    db;
    constructor(db) {
        this.db = db;
    }
    all() {
        return this.db.sql('SELECT * FROM type').then(rows => {
            return rows.map((typeRow) => AccountType.createFromRaw(typeRow.id.toString(), typeRow.name, typeRow.rate, typeRow.limitByClient, typeRow.description));
        });
    }
    async getOrSave(name, accountType) {
        ...
        if (existingTypes.length > 0) {
            ...
            return AccountType.createFromRaw(typeRow.id.toString(), typeRow.name, typeRow.rate, typeRow.limitByClient, typeRow.description);
        }
    ...
    async save(accountType) {
        ...
        return AccountType.createFromRaw(savedAccountTypeId, accountType.name, accountType.rate, accountType.limitByClient, accountType.description);
    }
