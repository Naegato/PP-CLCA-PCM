...
export class Iban {
    ...
    static create(value) {
        const MOD97_EXPECTED_RESULT = 1;
        const ASCII_UPPERCASE_A = 65;
        const LETTER_TO_NUMBER_OFFSET = 10;
        ...
        const convertedIban = rearrangedIban.replace(/[A-Z]/g, letter => (letter.charCodeAt(0) - ASCII_UPPERCASE_A + LETTER_TO_NUMBER_OFFSET).toString());
    ...
    static generate(bankCode, branchCode, accountNumber) {
        const ASCII_UPPERCASE_A = 65;
        const LETTER_TO_NUMBER_OFFSET = 10;
        const MOD97_DIVISOR = 97n;
        const IBAN_MOD97_TARGET = 98;
        // validate lengths
        ...
        // calculate RIB key
        const ribNumeric = BigInt(`${bankCode}${branchCode}${accountNumber}`.replace(/[A-Z]/gi, c => (c.toUpperCase().charCodeAt(0) - ASCII_UPPERCASE_A + LETTER_TO_NUMBER_OFFSET).toString()));
        const ribKey = (MOD97_DIVISOR - (ribNumeric % MOD97_DIVISOR)).toString().padStart(FRENCH_IBAN_ATTRIBUTES.CHECK_DIGITS_LENGTH, '0');
        // build BBAN
        ...
        const convertedIban = ibanWithPlaceholder.replace(/[A-Z]/g, letter => (letter.charCodeAt(0) - ASCII_UPPERCASE_A + LETTER_TO_NUMBER_OFFSET).toString());
