...
export class ClientMatchStockOrder {
    ...
    accountRepository;
    portfolioRepository;
    constructor(stockOrderRepository, accountRepository, portfolioRepository) {
    ...
    async processMatches(currentOrder, oppositeOrders, matchedQuantityAccumulator = 0) {
        ...
        const buyerDebit = Transaction.create(buyerAccount, tradeAmount + TRADING_FEE, `Purchase of ${tradeQuantity} ${currentOrder.stock.symbol} shares (including trading fee of ${TRADING_FEE}€)`);
        const sellerCredit = Transaction.create(sellerAccount, tradeAmount, `Sale of ${tradeQuantity} ${currentOrder.stock.symbol} shares`);
        const sellerFee = Transaction.create(sellerAccount, TRADING_FEE, `Trading fee for sale of ${currentOrder.stock.symbol} shares`);
        ...
        return this.processMatches(updatedCurrentOrder, remainingOppositeOrders, matchedQuantityAccumulator + tradeQuantity);
    }
}
//# sourceMappingURL=client-match-stock-order.js.map