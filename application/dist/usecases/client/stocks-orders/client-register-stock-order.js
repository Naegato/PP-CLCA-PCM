...
export class ClientRegisterStockOrder {
    ...
    stockRepository;
    matchStockOrder;
    constructor(stockOrderRepository, stockRepository, matchStockOrder) {
        this.stockOrderRepository = stockOrderRepository;
        this.stockRepository = stockRepository;
        this.matchStockOrder = matchStockOrder;
    }
    async execute(account, stockId, side, price, quantity) {
        ...
        if (side === OrderSide.BUY) {
            ...
            if (account.balance < totalCost) {
                return new ClientRegisterStockOrderError(`Insufficient balance to place buy order. Required: ${totalCost}, Available: ${account.balance}`);
            }
        }
        if (side === OrderSide.SELL) {
            ...
            if (availableToSell < quantity) {
                return new ClientRegisterStockOrderError(`Insufficient stock to sell. Owned: ${ownedQuantity}, Committed to sell: ${committedToSell}, Available: ${availableToSell}, Requested: ${quantity}.`);
            }
            if (account.balance < TRADING_FEE) {
                return new ClientRegisterStockOrderError(`Insufficient balance to cover the ${TRADING_FEE}€ fee for this sell order.`);
            }
        }
...
//# sourceMappingURL=client-register-stock-order.js.map